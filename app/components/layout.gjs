...
class Repo {
	data = null;
	load = () => {
		this.data = JSON.parse(localStorage.getItem('todos') || '[]').reduce((data, todo) => {
...
const App = <template>
	...
	<section class="todoapp">
		<header class="header"><h1>todos</h1><input
				class="new-todo"
...
let Footer = <template>
	<footer class="footer">
		...
		<span class="todo-count"><strong>{{repo.active.length}}</strong>
		...
		<ul class="filters">
			...
			<li>
				<a
					href="#"
					class={{if (isCompleted) "selected"}}
					{{on "click" showCompleted}}
				>{{completed}}</a>
			</li>
		</ul>
